#=====================================================================================
# Multiavatar v1.0 (Ruby version)
#=====================================================================================
# Package:   Multiavatar-Ruby
# Author:    Donovan Allen (http://roguesignal.io/)
# Github:    https://github.com/RogueSignalio/multiavatar-ruby
#
# Based on the Multiavatar project:
...
class Multiavatar
  ...
  # Multiavatar.generate(avatar_id, [sans_env], [ver])
  def self.generate(avatar_id=nil, sans_env=false, ver={})
    my_a = self.new(avatar_id, sans_env, ver)
    return my_a.avatar
  end

  # ma = Multiavatar.new(avatar_id, [sans_env], [ver])
  # ma.avatar
  # ma.generate(some_other_id)
  def initialize(avatar_id=nil, sans_env=false, ver={})
    @avatar_id = avatar_id || SecureRandom.hex(12)
    @sans_env = sans_env
    @ver = ver    
    @avatar = self.generate(@avatar_id, @sans_env, @ver)
    return self
  end
  ...
  def get_final(part, partV, theme, themes, shapes)
    colors = themes[partV.to_sym][theme.to_sym][part.to_sym];
    svg_string = shapes[partV.to_s][part.to_s];

    results = svg_string.scan(/#.*?;/)
    i = 0;
    results.each{|v|
      svg_string.gsub!(v,colors[i])
      i = i + 1
    }
  ...
  def generate(avatar_id=nil, sans_env=false, ver={})
    avatar_id = SecureRandom.hex(12) if !avatar_id or avatar_id.length == 0

    my_themes = self.themes()
    my_shapes = self.shapes()

    sha256Hash = Digest::SHA256.hexdigest(avatar_id);
    my_base = sha256Hash.gsub(/\D/,'')

    # Define the parts
    p = {
      env: (0.47 * my_base[0..1].to_i).round,
      clo: (0.47 * my_base[2..3].to_i).round,
      head: (0.47 * my_base[4..5].to_i).round,
      mouth: (0.47 * my_base[6..7].to_i).round,
      eyes: (0.47 * my_base[8..9].to_i).round,
      top: (0.47 * my_base[10..11].to_i).round,
    }
    ...
    p.each_pair{|key,part|
      ...
      # this doesn't make any sense to me... replace all parts with one part???
      # Seems like it should be ver[<part_name>] = { part: <dd>, theme: <A-C> }
      # original PHP:
      # if ($ver != null) {
      #     $partV = $ver['part'];
      #     $theme = $ver['theme'];
      ...
      final[key] =  get_final(key, partV, theme, my_themes, my_shapes);
    }

    avatar = "#{svgStart()}#{final[:env]}#{final[:head]}#{final[:clo]}#{final[:top]}#{final[:eyes]}#{final[:mouth]}#{svgEnd()}"
  ...
  # We store this as a instance var, since it is re-used a bunch.
  def shp_env
    @shp_env ||= '<path d="M33.83,33.83a115.5,115.5,0,1,1,0,163.34,115.49,115.49,0,0,1,0-163.34Z" style="fill:#01;"/>' 
    @shp_env
  end 

  # We store this as a instance var, since it is re-used a bunch.
  def shp_head
    @shp_head ||= '<path d="m115.5 51.75a63.75 63.75 0 0 0-10.5 126.63v14.09a115.5 115.5 0 0 0-53.729 19.027 115.5 115.5 0 0 0 128.46 0 115.5 115.5 0 0 0-53.729-19.029v-14.084a63.75 63.75 0 0 0 53.25-62.881 63.75 63.75 0 0 0-63.65-63.75 63.75 63.75 0 0 0-0.09961 0z" style="fill:#000;"/>'
    @shp_head
  end  

  # We store this as a instance var, since it is re-used a bunch.
  def stroke
    @stroke ||= 'stroke-linecap:round;stroke-linejoin:round;stroke-width:'
    @stroke 
  end

  # Captured this a method to:
  # * Remove the horror of this in the middle of the generate method
  # * Allow garbage collection to remove it when generate is done
  def themes
    # 
    # Color themes
    # 
  ...
  def shapes
    ...
    sP['01']['head'] = shp_head;
    sP['01']['mouth'] = '<path d="m100.35 143.85a7.67 7.67 0 0 0 7.58 7.7v0a7.66 7.66 0 0 0 7.57-7.7 7.66 7.66 0 0 0 7.57 7.7v0a7.67 7.67 0 0 0 7.58-7.7" style="fill:none;'+stroke+'6.3998px;stroke:#333;"/>';
    sP['01']['eyes'] = '<path d="m78.73 111a10.9 10.9 0 0 1 15.19 0m43.16 0a10.9 10.9 0 0 1 15.19 0" style="fill:none;'+stroke+'6.1999px;stroke:#333;"/><path d="m79.804 123.74h7.07m57.273 0h7.05" style="fill:none;'+stroke+'5.9998px;stroke:#b8b8b8;"/>';
    ...
    sP['03']['head'] = shp_head;
    sP['03']['mouth'] = '<path d="m136.21 147.09a21.77 21.77 0 0 1-40.13 0z" style="fill:#fff;'+stroke+'3.4999px;stroke:#000;"/>';
    sP['03']['eyes'] = '<path d="m145.39 104.7-11.52 11.2h17.26m-65.52-11.2 11.52 11.2h-17.26" style="fill:none;'+stroke+'5.4998px;stroke:#000;"/>';
    ...
    sP['04']['eyes'] = '<path d="m131.64 114.09 7.5801-7.5801 7.5801 7.5801m-62.6 0 7.5801-7.5801 7.5799 7.5801" style="fill:none;'+stroke+'6.4998px;stroke:#000;"/>';
    ...
    sP['05']['head'] = shp_head;
    sP['05']['mouth'] = '<path d="m122.83 151.88a10.49 10.489 0 0 1-14.66 0" style="fill:none;'+stroke+'6.1996px;stroke:#333;"/>';
    sP['05']['eyes'] = '<path d="m70.959 94.985h35.031c2.4086 1e-5 4.3612 1.9523 4.3612 4.3606l-2.5864 17.511c-0.3515 2.3799-1.7218 4.3606-3.8457 4.3606h-30.9c-2.1239-1e-5 -3.8457-1.9523-3.8457-4.3606l-2.5864-17.511c1e-5 -2.4082 1.9526-4.3606 4.3612-4.3606z" style="fill:#1a1a1a;'+stroke+'3.0045px;stroke:#333;"/><path d="m160.05 94.985h-35.031c-2.4086 1e-5 -4.3612 1.9523-4.3612 4.3606l2.5864 17.511c0.35149 2.3799 1.7218 4.3606 3.8457 4.3606h30.9c2.1239-1e-5 3.8457-1.9523 3.8457-4.3606l2.5864-17.511c-1e-5 -2.4082-1.9526-4.3606-4.3612-4.3606z" style="fill:#1a1a1a;'+stroke+'3.0045px;stroke:#333;"/><path d="m90.607 102.35a4.6337 4.6332 0 1 0 4.6892 4.6337 4.6337 4.6332 0 0 0-4.6892-4.6337zm49.72 0a4.6337 4.6332 0 1 0 4.6444 4.6337 4.6337 4.6332 0 0 0-4.6444-4.6337z" style="fill:#1a1a1a;"/><path d="m70.66 94.985h-11.775" style="fill:none;'+stroke+'3.0045px;stroke:#333;"/><path d="m172.13 94.985h-19.484" style="fill:none;'+stroke+'3.0045px;stroke:#333;"/><path d="m109.32 106.2c4.2045-2.427 9.3036-1.913 12.353-0.0258" style="fill:none;'+stroke+'3.0045px;stroke:#333;"/><path d="m148.33 109.79-5.7626-8.2324" style="fill:none;'+stroke+'4;stroke:#fff;"/><path d="m156.27 105-2.403-3.4328" style="fill:none;'+stroke+'4;stroke:#fff;"/><path d="m82.748 114.34-8.9489-12.784" style="fill:none;'+stroke+'4;stroke:#fff;"/><path d="m91.408 109.79-5.7626-8.2324" style="fill:none;'+stroke+'4;stroke:#fff;"/>';
